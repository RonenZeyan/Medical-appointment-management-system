window.onload = async function () {
  let box_bottom = document.getElementById(
  ...
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/appointment/existing-appointment/${userId}`,
      {
        ...
        headers: {
          ...
          Authorization: `Bearer ${token}`, // Example token for authentication (if needed)
        },
      }
    );
    ...
    if (data) {
      ...
      box_bottom.appendChild(span); // Or append to any other element

      box_bottom_span.textContent = `לצפייה או ביטול לחץ כאן`;
    } else {
      // If login failed, show an error message

      span.textContent = "*לא קיים תור, תרצה לעשות תור";
    }
  } catch (error) {
